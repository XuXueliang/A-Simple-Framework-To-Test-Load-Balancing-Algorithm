...
//plugin gradingDialog	
 ;(function ($) {
	   $.fn.gradingDialog=function(settings){
		   ...
		   function dialogInlit(){       //dialog 初始化
			   ...
			   var trs='<tr><td>评分：<input type="text" placeholder="请为实验报告打分"/></td></tr>'
			          +'<tr><td>评语：<textarea placeholder="请输入评语"></textarea></td></tr>';
			   $("body").append("<div class=\"mask\"></div>");
			   $("body").append('<div class="dialog" id="grading"></div>');
			   $(".dialog").append('<div class="title"><a class="close" href="#"><i class="icon-remove icon-white"></i><a>'+config.title+'</div>');
			   $(".dialog").append('<div class="content"><table></table></div>');
			   $(".dialog").append('<div class="bottom"><input id="cancelButton" type="button" value="取消" class="btn"/>&nbsp;&nbsp;<input id="confirmButton" type="button" value="提交" class="btn"/></div>');
			   $("#grading .content table").append(trs);
			   
			   $("#confirmButton").click(function() {//注册确定按钮点击事件
			        var str=$("#grading .content input").val();
					    str+=","+$("#grading .content textarea").val();
					config.onConfirm2(str); //添加onConfirm2动作
					})
				}	
		   ...
		   return $(this).each(function(){
			   // TO Do myCode
			   dialogInlit();      //dialog 初始化
			   dialogOperation(config,"#grading"); //dialog 操作
			   });
		   }
	  })(jQuery);		    	  